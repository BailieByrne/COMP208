...
int main(int argc, char *argv[]) {
    ...
    // record the starts time of the entire programs execution
    ...
    // recording the start time of the monte carlo simulation loop
    ...
    // recoring the end time of the mc sim loop
    auto end_sim = std::chrono::high_resolution_clock::now();
    
    csv_file.close();

    // recording the start of the AI prediction phase
    ...
    // recoding the end time of the AI prediction phase
    ...
    // final time stamp of the entire execution code
    auto end_total = std::chrono::high_resolution_clock::now();
    // calculating simulation times
    double sim_time = std::chrono::duration<double>(end_sim - start_sim).count(); //total sim time
    double ai_time = std::chrono::duration<double>(end_ai - start_ai).count(); //total ai time
    double total_time = std::chrono::duration<double>(end_total - start_total).count(); //total runtime of the entire program
    // open text file to store results 
    std::ofstream log("timing.txt");
    log << std::fixed << std::setprecision(8);

    log << "Simulation: " << sim_time << "s\n";
    log << "AI: " << ai_time << "s\n";
