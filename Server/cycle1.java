...
public class Cycle1 {
    ...
    private void sendTick(WebSocket socket, List<String> stockList, int step) {
        for (String ticker : stockList) {
            try {
                ...
                socket.send(createStockPacket(ticker, price, csvTick));
            } catch (Exception e) {
    ...
    private void startCoffeeMode(WebSocket socket, int clientID, List<String> stockList) {
        coffeeActive    = true;
    ...
    public void run(WebSocket socket, int clientID, List<String> stockList) {
